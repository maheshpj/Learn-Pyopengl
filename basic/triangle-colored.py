...
class ColoredTriangle:
    ...
    def run(self):
        ...
        # Setup Vertex VBO
        print >> sys.stderr, "Generate Vertex VBO"
        vbo = glGenBuffers(1)
        glBindBuffer(GL_ARRAY_BUFFER, vbo)
        glBufferData(GL_ARRAY_BUFFER, 9 * len(self.vertex_data),
                     self.vertex_data, GL_STATIC_DRAW)

        # Setup Colors VBO
        print >> sys.stderr, "Generate Colors VBO"
        cvbo = glGenBuffers(1)
        glBindBuffer(GL_ARRAY_BUFFER, cvbo)
        glBufferData(GL_ARRAY_BUFFER, 9 * len(self.colors_data),
                     self.colors_data, GL_STATIC_DRAW)
